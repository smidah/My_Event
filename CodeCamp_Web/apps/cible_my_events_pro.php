...
if ($req_utilisateur->execute(array($_SESSION['login'])))
{
	...
	if ($req_recup_id_event->execute(array($donnees_utilisateur['id'])))
	{
		...
		{	
			...
			$req_recup_event->execute(array($id_evenement['id_evenement'], 0));
		...
		if (!isset($verif_2)) 
		{
			echo '<h4 class="text-danger"><i class="fa fa-info-circle"></i> Vous ne participez à aucun Event Pro !</h4>';
		}
	}
}
